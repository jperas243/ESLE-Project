...
ssh -o StrictHostKeyChecking=accept-new $SERVER_IP_4 sudo docker run -d --name=roach1 --hostname=roach1 -p 26258:26258 -p 26257:26257 -p 26357:26357 -p 8080:8080 cockroachdb/cockroach:v23.1.11 start --advertise-addr=$SERVER_IP_4 --join=$SERVER_IP_1,$SERVER_IP_2,$SERVER_IP_3,$SERVER_IP_4,$SERVER_IP_5 --cache=$CACHE --max-sql-memory=$MAX_SQL_MEMORY --insecure 
...
CONNECTION_STRING_TOT="${CONNECT_STRING1} ${CONNECT_STRING2} ${CONNECT_STRING3} ${CONNECT_STRING4} ${CONNECT_STRING5}" # This variable counts as 7 variables when using it
...
sh run_benchmark_5.sh $SERVER_IP_BENCHMARK $CONNECTION_STRING_TOT $EXPERIMENT_NUMBER
sh run_benchmark_5.sh $SERVER_IP_BENCHMARK $CONNECTION_STRING_TOT $EXPERIMENT_NUMBER
sh run_benchmark_5.sh $SERVER_IP_BENCHMARK $CONNECTION_STRING_TOT $EXPERIMENT_NUMBER
sh run_benchmark_5.sh $SERVER_IP_BENCHMARK $CONNECTION_STRING_TOT $EXPERIMENT_NUMBER
sh run_benchmark_5.sh $SERVER_IP_BENCHMARK $CONNECTION_STRING_TOT $EXPERIMENT_NUMBER
