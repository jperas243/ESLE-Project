...
ssh -o StrictHostKeyChecking=accept-new $1 sudo docker run --rm --name=roach1 --hostname=roach1 -p 26257:26257 -p 26357:26357 -p 8080:8080 cockroachdb/cockroach:v23.1.11 workload run kv --concurrency=256 --duration=1m --display-every=30s --tolerate-errors $2 $3 $4 $5 $6 > ./benchmarks/Experiment-$7/benchmark-nodes1-test-$(date +%F_%H-%M-%S)""
